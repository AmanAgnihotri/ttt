...
func (s *session) GetUser(userID user.ID) (*entity.User, error) {
...
func (s *session) GetGame(gameCode game.Code) (*entity.Game, error) {
	...
	err := s.gameCollection.FindOne(s.context, filter, opts).Decode(&dao)
	if err != nil {
...
func (s *session) Set(game *entity.Game, user *entity.User) error {
	...
	group.Go(func() error {
		update := bson.M{
			"$set": bson.M{
				schema.UpdateTime: game.UpdateTime,
				schema.ExpiryTime: game.ExpiryTime,
			},
			"$push": bson.M{
				schema.Users: game.Users[1],
			},
		}
